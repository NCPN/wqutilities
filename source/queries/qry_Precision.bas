...
Where ="(((tblProjects.ProjectID)=\"NCPN_UTE\") AND ((tblVisits.START_DATE) Between #10/"
    "1/2011# And #9/30/2012#) AND ((tblResults.PRECISION) Is Not Null) AND ((IIf(IsNu"
    "meric([RESULT_TEXT]) And IsNumeric([Precision]),(CDbl([Precision])/(CDbl([RESULT"
    "_TEXT]))*100),2))>1) AND ((tblResults.VALUE_STATUS)=\"P\") AND ((tblCharacterist"
    "ics.FIELD_LAB)=\"Field\"))"
Begin InputTables
    Name ="tblProjects"
    Name ="tblLocations"
...
Begin OutputColumns
    ...
    Alias ="AMS+%"
    Expression ="IIf(IsNumeric([RESULT_TEXT]) And IsNumeric([Precision]),(CDbl([Precision])/(CDbl"
        "([RESULT_TEXT]))*100),2)"
...
Begin
    ...
    Begin
        ...
        dbLong "AggregateType" ="-1"
        dbBinary "GUID" = Begin
            0x0082670234bd230020bd2300bc6daf01
        End
    End
    Begin
