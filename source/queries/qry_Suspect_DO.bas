dbMemo "SQL" ="SELECT tblProjects.ProjectID, tblLocations.StationID, tblLocations.[Station Name"
    ...
    "tween #10/1/2011# And #9/30/2012#) AND ((tblCharacteristics.DISPLAY_NAME)=\"Diss"
    "olved oxygen (DO)\") AND ((tblResults.DETECTION_CONDITION)<>\"*Not Reported\") A"
    "ND ((tblResults.VALUE_STATUS)=\"P\") AND ((IIf(IsNumeric([RESULT_TEXT]),CDbl([RE"
    "SULT_TEXT]),-999))<5 Or (IIf(IsNumeric([RESULT_TEXT]),CDbl([RESULT_TEXT]),-999))"
    ">13))\015\012ORDER BY tblLocations.StationID, tblVisits.START_DATE, tblCharacter"
    "istics.DISPLAY_NAME;\015\012"
dbMemo "Connect" =""
